...
struct Btoggler8Widget : ModuleWidget {
	Btoggler8Widget(Btoggler8* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/Btoggler8.svg")));
		...
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(7, 21)), module, Btoggler8::CLOCK_INPUT));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(23, 21)), module, Btoggler8::FADE_PARAMS));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(38.7, 21)), module, Btoggler8::RSTALL_INPUT));
		
		float x = 8.9;
		float y = 10.5;
		for (int i=0;i<8;i++) {
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.1, 42+(i*y))), module, Btoggler8::ARM_INPUT+i));
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.1+x, 42+(i*y))), module, Btoggler8::IN_INPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.1+(2*x), 42+(i*y))), module, Btoggler8::OUT_OUTPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.1+(3*x), 42+(i*y))), module, Btoggler8::GATE_OUTPUT+i));
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.1+(4*x), 42+(i*y))), module, Btoggler8::RST_INPUT+i));
		}
	}
};


Model* modelBtoggler8 = createModel<Btoggler8, Btoggler8Widget>("Btoggler8");