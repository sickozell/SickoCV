...
struct BtogglerWidget : ModuleWidget {
	BtogglerWidget(Btoggler* module) {
		...
		float y = 10.5;
		for (int i=0;i<8;i++) {
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.1, 42+(i*y))), module, Btoggler::ARM_INPUT+i));
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.1+x, 42+(i*y))), module, Btoggler::IN_INPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.1+(2*x), 42+(i*y))), module, Btoggler::OUT_OUTPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.1+(3*x), 42+(i*y))), module, Btoggler::GATE_OUTPUT+i));
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.1+(4*x), 42+(i*y))), module, Btoggler::RST_INPUT+i));
		}
	}
};
