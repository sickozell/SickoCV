...
struct BtogglerPlusWidget : ModuleWidget {
	BtogglerPlusWidget(BtogglerPlus* module) {
		...
		float y = 10.5f;
		for (int i=0;i<8;i++) {
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.7, 40+(i*y))), module, BtogglerPlus::ARM_INPUT+i));
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.7+x, 40+(i*y))), module, BtogglerPlus::IN_INPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.7+(2*x), 40+(i*y))), module, BtogglerPlus::OUT_OUTPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.7+(3*x), 40+(i*y))), module, BtogglerPlus::GATE_OUTPUT+i));
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(5.7+(4*x), 40+(i*y))), module, BtogglerPlus::WARN_OUTPUT+i));
			addInput(createInputCentered<PJ301MPort>(mm2px(Vec(5.7+(5*x), 40+(i*y))), module, BtogglerPlus::RST_INPUT+i));
		}
	}
};
