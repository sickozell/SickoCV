...
In Loop Mode a trigger starts the loop, another trigger stops it.

Using a multichannel trigger cable, envelopes and outputs are generated by triggering individual channels.


#### RIGHT-CLICK MENU
- **LVL knob -> ENV out**: this turns the LVL knob into the attenuator of ENV out
- **Restart Loop on Load** (ticked by default): this works only with single channel trigger cable. It keeps looping when loading Rack, if Rack was closed while looping. Using polyphony trigger cable the loops don't restart when Rack is loaded.
- **Add Expander**: this adds one (or more) 'eX' expander module to add more input/outputs controlled by one single adMini
